...
trait HasFilters
{
    ...
    public function filterByJsonMultipleLang(Builder $q, $attribute, $value)
    {
        ...
        $q = isset($value) ? $q->whereRaw("JSON_SEARCH($attribute, 'all', ?) IS NOT NULL", ["%{$value}%"]) : $q;
        return $q;
    }

    public function filterByJsonMultipleLangLower(Builder $q, $attribute, $value)
    {

        $q = isset($value) ? $q->whereRaw("JSON_SEARCH(LOWER($attribute), 'all', LOWER(?)) IS NOT NULL", ["%{$value}%"]) : $q;
        return $q;
    }

    public function filterMultipleLang(Builder $q, $attribute, $value)
    {

        $q =isset($value) ? $q->where( function ($inner) use ($attribute, $value ) {
            $inner->where($attribute."->ar","LIKE","%{$value}%")
            ->orWhere($attribute."->en","LIKE","%{$value}%");
        }):$q;
